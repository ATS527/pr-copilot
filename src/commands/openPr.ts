...
export async function openPr(dependencies: OpenPrDependencies): Promise<OpenPrResult | undefined> {
  ...
  if (provider === "gitlab") {
    await vscode.window.showInformationMessage("GitLab support is planned next. Switching back to GitHub is recommended for now.");
    return undefined;
  }
  ...
  try {
    pullRequests = await dependencies.githubService.listPullRequests(repository.owner, repository.repo, token);
  } catch (error) {
